		...
		try {
			...
			List<ControllerAdviceInfo> controllerAdviceInfosNotInThisBean = controllerAdviceInfos.stream()
					.filter(controllerAdviceInfo ->
							// FIX: Spring Boot 3.4.x 的 ControllerAdviceBean 没有提供只含有 bean 的构造方法
							getControllerAdviceBean(controllerAdviceBeans, controllerAdviceInfo.getControllerAdvice()).isApplicableToBeanType(beanType))
