...
public interface IBaseService<T extends BaseEntity> extends IService<T> {
     ...
     * @since 2024/12/17 21:00
     */
    default List<T> listByParams(Map<String, Object> queryParams) {
        return listByMap(queryParams);
    }
     ...
     */
    default List<T> findByTenantId(String tenantId) {
        return list(new QueryWrapper<T>().eq("tenant_id", tenantId));
    }
