...
public interface CustomerRegionUserMapper extends IBaseMapper<CustomerRegionUser> {
    ...
    @Select("""
            <script>
                SELECT u.id,
                    c.id as 'customer.id', c.name as 'customer.name', c.serial_no as 'customer.serial_no',
                ...
                FROM t_crm_customer_region_user u
                LEFT JOIN t_crm_customer_region r ON u.region_id = r.id
                LEFT JOIN t_crm_customer c ON u.customer_id = c.id
                ...
                <where>
                    ...
                    AND u.deleted = 0 AND u.tenant_id = #{tenantId}
                </where>
            </script>
            """)
